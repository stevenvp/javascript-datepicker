(function (window) {
    ...
    function WinkelCalendar(options) {
        ...
        // these are human-readable month name labels, in order
        cal_months_labels = moment.months();
        cal_months_labels_short = moment.monthsShort();

        // moment weekdays function does not support the correct order
        // monday -> sunday instead of sunday -> monday order
        for (var i = 0; i < 7; i++) {
            cal_days_labels.push(moment().weekday(i).format('dd'));
            cal_full_days_lables.push(moment().weekday(i).format('dddd'));
        }
    ...
    WinkelCalendar.prototype.createDaysView = function () {
        ...
        // get first day of month
        var firstDay = new Date(year, month, 1);
        var startingDay = moment(firstDay).weekday();
